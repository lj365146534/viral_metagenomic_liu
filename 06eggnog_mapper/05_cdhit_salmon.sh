...
### 3.2.3 基因定量salmon
    ...
    salmon index \
      -t result/NR/full_nucleotide.fa \
      -p 20 \
      -i temp/salmon/full_index 
...
do
	salmon quant -i temp/salmon/index -l A -p 20 --meta -r ../01trim/bowtie/nohost_silva_${i}.fastq.gz -o temp/salmon/${i}.quant
done
...
head -n3 result/salmon/full.gene.*
